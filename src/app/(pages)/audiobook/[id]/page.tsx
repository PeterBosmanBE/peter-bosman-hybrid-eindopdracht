...
export default function Details({ params }: { params: Promise<PageParams> }) {
  ...
  const addToLibraryMutation = useMutation(
    orpc.library.addToLibrary.mutationOptions({
      onSuccess: () => {
        ...
        queryClient.invalidateQueries({
          queryKey: orpc.library.listLibrary.queryKey(),
        });
  ...
  const isLibraryActionPending =
    addToLibraryMutation.isPending || removeFromLibraryMutation.isPending;
  ...
  const displayDuration = useMemo(() => {
    ...
    const totalSeconds = content.chapters.reduce((sum, chapter) => {
      ...
      return (
        sum + parseDurationToSeconds(resolved || chapter.duration || "00:00")
      );
    }, 0);
  ...
  if (
    detailQuery.isError ||
    (detailQuery.isFetched && !detailQuery.data) ||
    !content
  ) {
    return <NotFound />;
  }
  ...
  return (
    ...
    >
      <section
        className="border-b"
        style={{ background: "#232F3E", borderColor: "#37475A" }}
      >
            ...
            <div>
              ...
              >
                <button
                  ...
                  style={{
                    color: activeTab === "chapters" ? "#232F3E" : "#666666",
                  }}
                >
