...
export default function ListenPodcast({
  ...
  const currentEpisodeIndex = useMemo(() => {
    ...
    const foundIndex = podcastEpisodes.findIndex(
  ...
  const episode =
    currentEpisodeIndex >= 0 ? podcastEpisodes[currentEpisodeIndex] : undefined;
  const currentEpisodeId = episode?.id || "";
  const { data: dbBookmarks } = useQuery({
    ...orpc.bookmarks.getBookmarks.queryOptions({
      input: { contentId: currentEpisodeId, contentType: "podcast" },
    }),
  ...
  useEffect(() => {
    ...
    const setAudioEnd = () => {
      ...
      if (
        ...
        currentEpisodeIndex < podcastEpisodes.length - 1
      ) {
            ...
            <button
              onClick={() => {
                ...
                setSelectedEpisodeId(
                  podcastEpisodes[currentEpisodeIndex - 1].id,
                );
              }}
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <p
                            className={`text-sm font-semibold truncate ${currentEpisodeIndex === index ? "text-white" : ""}`}
                            style={{
                              color:
                                currentEpisodeIndex === index
                                  ? "white"
